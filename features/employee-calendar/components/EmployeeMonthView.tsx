...
export default function EmployeeMonthView({
  ...
  const mobileCells = monthCells.filter(
  ...
  return (
    <div className="space-y-3">
        ...
        {mobileCells.map((cell) => {
          ...
          return (
            <div
              key={`mobile-${isoDate}`}
              className={cn(
                "rounded-2xl border border-slate-200 bg-white p-3 dark:border-neutral-700 dark:bg-neutral-900",
            ...
            >
              ...
              <div className="mt-2 space-y-1.5">
                {dayShifts.length === 0 ? (
                  <p className="rounded-lg border border-dashed border-slate-200 px-2 py-2 text-xs text-slate-500 dark:border-neutral-700 dark:text-slate-400">
                    Chua co ca
                  </p>
                ) : (
      ...
      <div className="hidden overflow-auto sm:block">
        <div className="grid min-w-220 grid-cols-7 gap-1 rounded-2xl border border-slate-200 bg-slate-100 p-1 dark:border-neutral-700 dark:bg-neutral-800">
          {DAY_NAMES.map((day) => (
            <div
              key={day}
