...
export default function EmployeeList({ employees, onEdit, onDelete }: EmployeeListProps) {
  ...
  const SortIcon = ({ field }: { field: SortField }) => (
    <span className="ml-1 inline-flex flex-col">
      <svg
        className={`w-3 h-3 ${sortField === field && sortOrder === "asc" ? "text-purple-500" : "text-neutral-300"}`}
        fill="currentColor"
        viewBox="0 0 24 24"
      >
      ...
      <svg
        className={`w-3 h-3 -mt-1 ${sortField === field && sortOrder === "desc" ? "text-purple-500" : "text-neutral-300"}`}
        fill="currentColor"
        viewBox="0 0 24 24"
      >
                ...
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-r from-[#402093] via-[#8f58e4] to-[#5e34b7] flex items-center justify-center text-white font-semibold text-sm">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        className="p-2 hover:bg-purple-100 dark:hover:bg-purple-900/30 rounded-lg transition-colors text-purple-600 dark:text-purple-400"
                        title="Chỉnh sửa"
                      >
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
              <button
                ...
                className={`w-8 h-8 rounded-lg text-sm font-medium transition-colors ${
                  page === currentPage
                    ? "bg-gradient-to-r from-[#402093] via-[#8f58e4] to-[#5e34b7] text-white"
