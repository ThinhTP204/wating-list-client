...
export default function MyCalendar() {
  ...
  const { start: startFetch, end: endFetch } = useMemo(() => {
    ...
    const end = new Date(baseYear, baseMonth + 1, 0);
    return {
  ...
  const currentMonthShifts = shifts.filter((s) => {
    ...
    return (
      shiftDate.getMonth() === baseDate.getMonth() &&
      shiftDate.getFullYear() === baseDate.getFullYear()
    );
  });
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
              {totalShifts} ca · {publishedShifts} đã công bố{" "}
              {viewMode === "week" ? "trong tuần" : "trong tháng"}
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 border border-slate-200 dark:border-neutral-800 rounded-lg p-1 bg-white/50 dark:bg-neutral-900/50 backdrop-blur-[2px]">
            ...
            <div className="flex items-center gap-1 pr-1">
              <Button
                ...
                size="sm"
                onClick={handlePrev}
                className="h-8 w-8 p-0 hover:bg-slate-200/50 dark:hover:bg-neutral-800"
              >
                ...
                <div key={dateStr} className="flex flex-col gap-2 relative">
                  ...
                  <div
                    className={cn(
                      ...
                      isToday
                        ? "bg-blue-50/80 dark:bg-blue-900/40 border-blue-200/80 dark:border-blue-700/50"
                        : isWeekend
                          ? "bg-slate-50/70 dark:bg-neutral-800/40 border-slate-100 dark:border-neutral-800/80"
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between w-full overflow-hidden gap-2">
                            ...
                            {isDraft && (
                              <FileEdit className="w-3.5 h-3.5 flex-shrink-0 opacity-70" />
                            )}
                            {isAbsent && (
          ...
          <>
            <div className="grid grid-cols-7 gap-1 border-b border-slate-200 dark:border-neutral-800 pb-2 flex-shrink-0">
              {DAY_NAMES.map((d) => (
                <div
                  key={d}
                  className="text-center text-xs font-bold uppercase text-slate-400 dark:text-slate-500 py-1"
                >
                  {d}
                </div>
              ))}
              ...
              {monthGrid.map((day, idx) => {
                if (!day) {
                  return (
                    <div
                      key={`empty-${idx}`}
                      className="bg-white/50 dark:bg-neutral-900/50 min-h-[80px]"
                    />
                  );
                }
                const dateStr = toISODate(day);
        ...
        <div className="flex items-center gap-4 pt-1 opacity-80 text-[11px] font-medium text-slate-500 dark:text-slate-400 pb-1 w-full justify-between flex-shrink-0">
          <div className="flex gap-4">
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 bg-white dark:bg-neutral-900 border border-slate-300 dark:border-neutral-600 rounded-full" />{" "}
              Đã công bố
            </span>
            <span className="flex items-center gap-1.5">
              <FileEdit className="w-3 h-3 opacity-70" /> Bản nháp
            </span>
          </div>
          <div className="flex gap-4">
